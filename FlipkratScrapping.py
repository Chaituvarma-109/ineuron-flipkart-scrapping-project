...
class FlipkartScrapper:
    ...
    def getLinkForExpectedReviewCount(self, expected_review, searchString):
        ...
        try:
            ...
            raise Exception(
                f"(getLinkForExpectedReviewCount) - Failed to retrieve the link for product having more than "
                f"expected count of review.\n" + str(
                    e))
    ...
    def getCustomerNamesAndReviewAge(self):
        """
        This function gets customer name for the review
        """
        try:
            locator = self.getLocatorsObject()
    ...
    def getExpectedCountForLooping(self, expected_review):
        """
        This function retrieves the total number of pages which should be searched for review
        """
        try:
    ...
    def getReviewDetailsForProduct(self):
        ...
        try:
            ...
            customer_name.append(
                self.separateCustomernameAndReviewAge(list_of_cust_name_and_review_age=cust_name_and_review_age)[0])
            review_age.append(
                self.separateCustomernameAndReviewAge(list_of_cust_name_and_review_age=cust_name_and_review_age)[1])
    ...
    def separateCustomernameAndReviewAge(self, list_of_cust_name_and_review_age):
        """
        This function separates the review age and customer name.
        """
        try:
            customer_name = list_of_cust_name_and_review_age[0::2]
            review_age = list_of_cust_name_and_review_age[1::2]
    ...
    def generateDataForColumnAndFrame(self, response):
        """
        This function generates data for column where only single data is presented.And then frames it in data frame.
        """
        try:
            data_frame1 = pd.DataFrame()
            flatten_rating = [j for i in response['ratings'] for j in i]
            for column_name, value in response.items():
                if column_name == 'product_searched' or column_name == 'product_name' or column_name == 'price' or \
                        column_name == 'discount_percent' or column_name == 'offer_details' or column_name == 'EMI':
                    list_value = []
